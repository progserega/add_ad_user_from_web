...
def main():
  ...
  global web_user_addr
  global web_user_host
  global web_user_name
  global log

  log.debug("view ad list from: IP: %(web_user_addr)s, auth_name: %(web_user_name)s," % {"web_user_addr":web_user_addr, "web_user_name":web_user_name} )
  ...
  l=ad.init(log)
  if l is None:
    log.error("error init ldap connection")
    return False
  users=ad.get_users(l)
  if users is None:
    log.error("ad.get_users()")
    return False
  comps=ad.get_computers(l)
  if comps is None:
    log.error("ad.get_computers()")
    return False
  ...
  #ip=users_from_db[account_name]["ip"]
    ...
    if fio in users_phones:
      ...
      if users_phones[fio]["email"] != "":
        if drsk_email.strip() != users_phones[fio]["email"].strip():
          ...
          log.debug("1c != userdb: '%s' != '%s'"%(users_phones[fio]["email"].strip(),drsk_email.strip()))
    ...
    if "mail" in user:
      if user["mail"].strip() != drsk_email.strip():
        email_differ = True
        log.debug("ad != 1c: '%s' != '%s'"%(user["mail"].strip(),drsk_email.strip()))
